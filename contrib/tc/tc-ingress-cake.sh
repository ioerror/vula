...
tc qdisc delete root dev $IFACE_TUN || true;
tc filter delete root dev $IFACE_TUN || true;

# We setup our qdisc using cake to do reasonable dynamic traffic shaping
tc qdisc replace dev $IFACE_TUN handle 1: root cake ethernet besteffort ingress;

# Rewrite peer IPv6 packets which would otherwise be silently dropped
tc filter add dev $IFACE_TUN parent 1: prio 1 protocol ipv6 \
