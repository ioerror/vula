...
tc qdisc del dev $IFACE_TUN ingress handle ffff: || true;
tc filter del dev $IFACE_TUN prio 1 || true;

# We setup our qdisc using HTB
tc qdisc add dev $IFACE_TUN ingress handle ffff:;

# Rewrite peer IPv6 packets which would otherwise be silently dropped
tc filter add dev $IFACE_TUN prio 1 protocol ipv6 parent ffff: \
  flower src_ip $v6_LL_PEER dst_ip $v6_LL_SELF \
  skip_hw \
